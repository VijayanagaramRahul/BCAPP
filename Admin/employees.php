...
<div id="main">
    ...
    if(isset($_POST['delete']))
    {
        ...
        $del="Delete From workers where loginid='$loginid'";
        $getdata=mysqli_query($conn,$del);
        if($getdata)
        {
            echo '<h3 align="center" style="color:darkgreen;padding:5px;">Deleted</h3>';
        }
        else
        {
            echo '<h3 align="center" style="color:red;padding:5px;">Not!Deleted</h3>';
...
<?php
    ...
    {
        $loginid=$_POST['loginid'];
        $update="UPDATE workers SET status='verified' where loginid='$loginid'";
        $getdata=mysqli_query($conn,$update);
        if($getdata)
        {
            echo '<h3 align="center" style="color:darkgreen;padding:5px;">Updated</h3>';
        }
        else
        {
            echo '<h3 align="center" style="color:red;padding:5px;">Not!Updated</h3>';
...
<?php
    ...
    while($row=mysqli_fetch_array($getdata, MYSQLI_ASSOC)){
        echo "
        <div style='background:#012461;margin-top:10px;colorwhite;padding:20px;'>
        <h6 style='color:yellow;'>
      Name : ".$row['name']."<br> Loginid :".$row['loginid']."<br>status:".$row['status']."</h6>
        <h4 align='center' style='color:white;'> Date of Register : ".$row['dateandtimeofjoined']."</h4>
        <form method='POST'> 
        <input type='hidden' name='loginid' value=".$row['loginid']." >
        
        <p align='center'><input type='submit' name='delete' value='Delete' class='btn btn-danger'></p>
        
        <p align='center'><input type='submit' name='completed' value='Verify' class='btn btn-success'></p>
        </form>
        </div>
        ";
        ...
        echo '<h3 align="center" style="color:Orange">NO orders are Found!</h3>';
    }
